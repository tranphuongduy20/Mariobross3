...
#define OBJECT_TYPE_CBRICKV2_MUSH	28
...
void PlayScene::ChooseMap(int whatMap)
{
	...
	if (idStage == 500)
	{
		...
		DebugOut(L"chon map world scene");
	}
	if (idStage == 1000)
	{
		player->Reset();
...
void PlayScene::_ParseSection_OBJECTS(string line)
{
	...
	case OBJECT_TYPE_CBRICKV2_MUSH:
	{
		obj = new CBrickV2(x, y, atof(tokens[4].c_str()), atof(tokens[5].c_str()));
		//obj->SetPosition(x, y);
		LPANIMATION_SET ani_set = animation_sets->Get(ani_set_id);
		CBrickV2* brick = dynamic_cast<CBrickV2*>(obj);
		brick->isMush = true;
		obj->SetAnimationSet(ani_set);
		listObjects.push_back(obj);
		DebugOut(L"[test] add cbrickV2 !\n");
